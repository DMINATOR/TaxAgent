...
{  
  ...
  public function logREQUEST()
  {
	  	...
	  	$this->lwrite('[REQUEST] ('.$_REQUEST.') <-'.$dump);
  }
  
  
   //logs all the request params
  public function logRESPONSE($action, $message)
  {
	  	ob_start();
  		var_dump($message);
  		$dump = ob_get_clean();
  
	  	$this->lwrite('[RESPONSE ('.$action.')]->'.$dump);
  }
  
  //logs all the request params
  public function logJSON($action, $message)
  {
	  	ob_start();
  		var_dump($message);
  		$dump = ob_get_clean();
  
	  	$this->lwrite('[JSON ('.$action.')]->'.$dump);
  }
  
  
  //logs all SQL messages
  public function logSQL( $message)
  {
	  	$this->lwrite('[SQL]->'.$message);
  }
  
  //logs Thread messages
  public function logAgent( $message)
  {
	  	$this->lwrite('[AGENT]->'.$message);
  }
